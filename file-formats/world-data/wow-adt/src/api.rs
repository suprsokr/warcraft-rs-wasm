...
pub fn parse_adt_with_metadata<R: Read + Seek>(reader: &mut R) -> Result<(ParsedAdt, AdtMetadata)> {
    // Discovery phase
    // Note: std::time::Instant is unsupported on wasm32-unknown-unknown and
    // panics there, so timing is disabled on that target.
    #[cfg(not(target_arch = "wasm32"))]
    ...
    let discovery_duration = Duration::ZERO;
    ...
    #[cfg(target_arch = "wasm32")]
    let parse_duration = Duration::ZERO;
